...
func TestWebhookOpsOpts_Run(t *testing.T) {
	...
	mockStore.
		EXPECT().
		CreateIntegration(opts.ProjectID, webhookIntegrationType, opts.newWebhookIntegration()).
		Return(expected, nil).
		Times(1)
...
func TestWebhookBuilder(t *testing.T) {
	test.CmdValidator(
		t,
		WebhookBuilder(),
		0,
		[]string{flag.URL, flag.Secret, flag.ProjectID},
	)
}
