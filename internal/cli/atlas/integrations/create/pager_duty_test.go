...
func TestPagerDutyOpts_Run(t *testing.T) {
	...
	mockStore.
		EXPECT().
		CreateIntegration(opts.ProjectID, pagerDutyIntegrationType, opts.newPagerDutyIntegration()).
		Return(expected, nil).
		Times(1)
...
func TestPagerDutyBuilder(t *testing.T) {
	test.CmdValidator(
		t,
		PagerDutyBuilder(),
		0,
		[]string{flag.ServiceKey, flag.ProjectID},
	)
}
