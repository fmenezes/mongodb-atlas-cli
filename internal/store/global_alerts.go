...
//go:generate mockgen  -destination=../mocks/mock_global_alerts.go -package=mocks github.com/fmenezes/mongodb-atlas-cli/v2/internal/store GlobalAlertLister
...
func (s *Store) GlobalAlerts(opts *atlas.AlertsListOptions) (*opsmngr.GlobalAlerts, error) {
	...
	case config.OpsManagerService:
		result, _, err := s.client.(*opsmngr.Client).GlobalAlerts.List(s.ctx, opts)
		return result, err
	default:
